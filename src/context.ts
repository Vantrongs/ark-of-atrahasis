...
export interface DocumentContext {
  ...
  setStyle(real: Element, property: string, value: string, action: () => void): void;
  addEventListener(
    real: Element,
    eventName: string,
...
class DocumentContextImplementation implements DocumentContext {
  ...
  constructor(root: ShadowRoot, ownerDocument: Document, options?: SafeDocumentOptions) {
    ...
    this.#AbortController = ownerDocument.defaultView!.AbortController;
  }
  ...
  setStyle(real: Element, property: string, value: string, action: () => void): void {
    this.nodeOperation(real, () => {
      ...
      this.#updateResources(
        entry,
        [{ resource: "style", slot: property, amount: utf8ByteLength(value) }],
        action,
      );
    });
  }
  ...
  #hasSafePlacement(entry: RegistryEntry): boolean {
    ...
    for (;;) {
      ...
      if (!parentEntry || parentEntry.state !== "active") return false;
      current = parent;
    }
  }
  ...
  #clearOwnedResources(entry: RegistryEntry): void {
    if (!(entry.real instanceof this.ownerDocument.defaultView!.Element)) return;
    for (const name of entry.resources.request.keys()) entry.real.removeAttribute(name);
    if (entry.resources.style.has("$stylesheet")) entry.real.textContent = "";
    if ([...entry.resources.style.keys()].some((name) => name !== "$stylesheet")) {
